...
def setup_heightmap(singleton, path):
    ...
    if name == "random":
        ...
        with open('temp/' + singleton.output_name + "_heightmap.txt", 'w') as f:
    ...
    if (name[0:-4] + "_" + str(singleton.border[0]) + "_" +
            str(singleton.border[1]) in os.listdir("temp/")):
        return 0
    ...
    import pickle
    with open('temp/' + name[0:-4] + "_" + str(singleton.border[0]) + "_" + str(singleton.border[1]),
              "wb") as f:
