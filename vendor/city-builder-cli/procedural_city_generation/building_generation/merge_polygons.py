...
def merge_polygons(polygons, textures, output_name):
    ...
    for polys in poly_group_by_texture:
        if len(polys) > 0:
            ...
            # TODO: Fix Scale!
            savelist = [[x.tolist() for x in M.allverts], M.allfaces, textures[ind].name, textures[ind].scale * 30,
                        textures[ind].shrinkwrap]
            mergedpolys.append(savelist)
        ind += 1

    print("Merging done, saving data structure")
    import pickle
    with open("outputs/" + output_name + ".txt", 'wb') as f:
        import sys
        if sys.version[0] == "2":
