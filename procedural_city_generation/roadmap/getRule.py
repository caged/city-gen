...
def getRule(vertex):
    ...
    i = int(singleton.img2.shape[0] - y * singleton.img2.shape[0])
    j = int(x * singleton.img2.shape[1])

    population_density = np.sqrt((singleton.img2[i][j][0]) / 255)
    if vertex.seed:
    ...
    if not vertex.minor_road:
        ...
        i = int(singleton.img.shape[0] - y * singleton.img.shape[0])
        j = int(x * singleton.img.shape[1])

        intrule = np.argmax(singleton.img[i][j])
        z = (0, 0)
