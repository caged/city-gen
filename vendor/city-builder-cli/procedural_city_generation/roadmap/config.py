...
def config():
    ...
    singleton.maxLength = max(singleton.radiallMax, singleton.gridlMax, singleton.organiclMax, singleton.minor_roadlMax,
                              singleton.seedlMax)
    ...
    with open('temp/' + singleton.output_name + "_densitymap.txt", 'w') as f:
    ...
    singleton.center = [np.array([singleton.border[0] * ((x[1] / singleton.img.shape[1]) - 0.5) * 2,
                                  singleton.border[1] * (
                                              ((singleton.img.shape[0] - x[0]) / singleton.img.shape[0]) - 0.5) * 2])
    ...
    singleton.global_lists.tree = KDTree(singleton.global_lists.coordsliste, leafsize=160)
